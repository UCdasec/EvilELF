...
from src.model import AvastNet,MalConv,FireEye
...
def train_loop(epochs,model,train_data,val_data,lr,model_path,device):
    ...
    for epoch in range(1,epochs+1):
        ...
        if valid_loss < best_val_loss:
            ...
            with open(args.log_file_path, 'a') as f:
                print(f'Checkpoint saved at: {epoch}, {model_path}',file=f)
...
def main_train():
    ...
    # train_label_table.index = train_label_table.index.str.upper()  # upper the string originally
    # train_label_table.index = train_label_table.index.str
    ...
    # test_label_table.index = test_label_table.index.str
    ...
    "load model"
    if args.model_name == 'AvastNet':
        model = AvastNet(vocab_size=256)
    elif args.model_name == 'malconv':
        model = MalConv(max_input_size=args.input_size, window_size=args.window_size)
    elif args.model_name == 'fireeye':
        model = FireEye(input_length=args.input_size, window_size=args.window_size)
    else:
        sys.exit('Select correct model name, [AvastNet,malconv,fireeye].')
